...
(defvar panache-module-dir (file-name-directory load-file-name) "directory of this module.")
...
(defun panache-cleanup-command(start end)
  ...
  (let ((coding-system-for-read 'utf-8)
    ...
    (message panache-module-dir)
    (message (format "pandoc --standalone -t gfm --lua-filter=%skeepTOC.lua" panache-module-dir))
    (shell-command-on-region
     ...
     (format "pandoc --standalone --atx-headers --wrap=auto --columns=80 --to=markdown_strict+pipe_tables+yaml_metadata_block+blank_before_header+header_attributes+blank_before_blockquote+fenced_code_blocks+fenced_code_attributes+definition_lists+table_captions+intraword_underscores+tex_math_dollars+raw_tex+shortcut_reference_links+bracketed_spans+auto_identifiers+link_attributes+escaped_line_breaks+bracketed_spans+link_attributes+strikeout --lua-filter=%skeepTOC.lua" panache-module-dir)
     (current-buffer)
     t
